import ABLandingPage from '@/components/ABLandingPage';
...
export default async function ABPage() {
            ...
            if (!leaderboardError && topScans) {
                const processedLeaderboard = topScans
                    ...
                    .reduce((acc: any[], current) => {
                        const existing = acc.find(item => item.domain === current.domain);
                        if (!existing) {
                            acc.push(current);
    ...
    return <ABLandingPage stats={stats} leaderboard={leaderboard} />;
}
